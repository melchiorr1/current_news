...
def get_articles(num=4):
    ...
    no_image = 'https://cdn.kibrispdr.org/data/753/no-image-available-png-6.jpg'
    for article in response.json()['results'][:num]:
        if article['media']:
            dic[article['title']] = [article['url'], article['media']
                                     [0]['media-metadata'][-1]['url'], article['byline']]
        else:
            dic[article['title']] = [article['url'], no_image, article['byline']]
...
def add():
    if request.method == 'POST':
        ...
        if Article.query.filter_by(title=title, user_id=current_user.id).first():
            flash("Article is already in a read later list", category='error')
            return jsonify({})

        new_article = Article(title=title, authors=authors,
                              url=url, media=img, user_id=current_user.id)
